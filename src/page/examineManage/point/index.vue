<template>
    <div class="fillcontain">
        <div class="search_container searchpoint">
            <el-form 
                :inline="true" 
                class="demo-form-inline search-form" :model="pointNow"> 
        ...
        <div class="table_container">
            <el-table v-loading="tableLoading"
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-tag v-for="item in lineList" v-if="item.lineId == scope.row.pointLine"  type="success">
                            {{ item.lineName }}
                        </el-tag>
                    </template>
                </el-table-column>
                 <el-table-column
                    ...
                    width="100">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.pointStat === 0" type="success">启用</el-tag>
                        <el-tag v-if="scope.row.pointStat === 1" type="danger">禁用</el-tag>
                    </template>
                </el-table-column>
                <el-table-column
                    prop="createBy"
            ...
            <el-dialog
                title="点位信息"
                :visible.sync="pointShow"
                width="30%">
                <el-form ref="form" :model="pointNow" :rules="rules" label-width="80px">
                    <el-form-item label="点位id" prop="pointId">
                        <el-input v-model="pointNow.pointId" :disabled="true"></el-input>
                    </el-form-item>
                    <el-form-item label="点位名" prop="pointName">
                        <el-input v-model="pointNow.pointName"></el-input>
                    </el-form-item>
                    <el-form-item label="点位描述" prop="pointDesc">
                        <el-input type="textarea" v-model="pointNow.pointDesc"></el-input>
                    </el-form-item>
                    <el-form-item label="所属线路" prop="pointLine">
                        <el-select v-model="pointNow.pointLine" placeholder="请选择" style="width: 100%">
                            <el-option v-for="item in lineList" :label="item.lineName" :value="item.lineId">{{ item.lineName }}</el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="点位状态" prop="pointStat">
                        <el-select v-model="pointNow.pointStat" placeholder="请选择" style="width: 100%">
                            <el-option label="启用" :value="0">启用</el-option>
                            <el-option label="禁用" :value="1">禁用</el-option>
                        </el-select>
                    </el-form-item>
                    <!-- <el-form-item label="创建人" prop="createBy">
                        <el-input v-model="pointNow.createBy" :disabled="true"></el-input>
                    </el-form-item>
                     <el-form-item label="创建时间">
                        <el-col :span="11">
                            <el-form-item prop="createTime">
                                <el-date-picker
                                    v-model="pointNow.createTime"
                    ...
                    <el-form-item label="更新人" prop="updateBy">
                        <el-input v-model="pointNow.updateBy" :disabled="true"></el-input>
                    </el-form-item>
                     <el-form-item label="更新时间">
                        <el-col :span="11">
                            <el-form-item prop="updateTime">
                                <el-date-picker
                                    v-model="pointNow.updateTime"
                                    format="yyyy-MM-dd HH:mm:ss"
                                    value-format="yyyy-MM-dd HH:mm:ss"
                                    type="datetime"
                                    placeholder="请选择日期时间"
    ...
    export default {
        data(){
            return {
                ...
                tableData:[],
                search_data: '',
                pointShow: false, //是否显示新增或编辑的弹出框
                pointNow: {},     //正在编辑的point
                pointNowBefore: null,   //编辑之前的partrol
                editpoint: false,
                rules: {           //定义校检方案
                    pointName: [
                        { required: true, message: '请输入点位名', trigger: 'change' }
                    ],
                    pointLine: [
                        { required: true, message: '请输选择点位所属线路', trigger: 'change' }
                    ],
                    pointStat: [
                        { required: true, message: '请选点位择状态', trigger: 'change' }
                    ]
                },
                chekcedData:[],                       //被选中的数据项
        ...
        methods: {
            ...
            edit(row) {
                this.pointNow = JSON.parse(JSON.stringify(row))
                this.pointNowBefore = row
                this.pointShow = true
                this.editpoint = true
            },
            batchDelete() {
                ...
                var points = []
                for(var point  of this.chekcedData) {
        ...
        　　　　　　const tHeader = ['pointId','pointTime','pointTask','pointStatu','pointStaff','pointDeal'];
        　　　　　　const filterVal = ['pointId', 'pointTime', 'pointTask', 'pointStatu', 'pointStaff', 'pointDeal'];
        　　　　　　const list = this.chekcedData;
        　　　　　　const data = this.formatJson(filterVal, list);
        　　　　　　export_json_to_excel(tHeader, data, 'point');
    　　　　    })
            },
            getData() {
               axios({
                    method: 'get',
                    url:'/point/all/' + this.page.pageNow + '/' + this.page.pageSize
                }).then(resp => {
                    if(resp.code === 200) {
            ...
            getSearchData() {
                axios({
                    method: 'get',
                    url:'/point/search/' + this.search_data + '/' + this.page.pageNow +'/' + this.page.pageSize,
                }).then(resp => {
                    if(resp.code === 200) {
                        this.tableData = resp.data.list
                        this.page = resp.data
